...
struct DashboardView: View {
    ...
    private var usageTint: Color { shell.usageAgent == "codex" ? Color(red: 0.44, green: 0.48, blue: 0.94) : Color(red: 0.85, green: 0.47, blue: 0.34) }
                ...
                LazyVStack(spacing: 0) {
                    ForEach(rows) { row in
                        DashboardCard(row: row, opening: model.navigation.opening == row.url.absoluteString,
                            open: { model.open(row) }, external: { model.openExternally(row) }, copy: { model.copyLink(row) })
...
struct DashboardCard: View {
    let row: DashboardRow; let opening: Bool; let open: () -> Void; let external: () -> Void; let copy: () -> Void
    ...
    var body: some View {
        Button(action: open) {
            ...
            .contextMenu { Button("Open in TaskHub", action: open); Button("Open in Browser", action: external); Button("Copy Link", action: copy) }
    }
