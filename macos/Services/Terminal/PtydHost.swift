...
struct PtydConfiguration: Sendable {
    ...
    static func current() throws -> Self {
        ...
        let data = argument("--data-dir") ?? env["TASKHUB_DATA_DIR"]
            ?? FileManager.default.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support/TaskHub").path
        let executable = argument("--ptyd-path").map { URL(fileURLWithPath: $0) }
            ?? Bundle.main.bundleURL.appendingPathComponent("Contents/Helpers/taskhub-ptyd")
        return Self(executable: executable, directory: URL(fileURLWithPath: data).appendingPathComponent("ptyd-native-spike"),
                    socketPath: try argument("--pty-socket") ?? env["TASKHUB_PTYD_SOCK"] ?? defaultSocket(environment: env))
    }
    ...
    static func defaultSocket(environment: [String: String]) throws -> String {
        ...
        // Temporary M1 isolation: the established app uses taskhub-ptyd.sock.
        return URL(fileURLWithPath: directory).appendingPathComponent("taskhub-native-ptyd.sock").path
    }
...
actor PtydHost {
    ...
    func connect(client: PtydClient) async throws -> PtyHello {
        ...
        environment["TASKHUB_PTYD_SOCK"] = configuration.socketPath
