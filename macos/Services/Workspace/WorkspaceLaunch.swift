...
struct WorkspaceLaunchCommand: Equatable, Sendable {
    ...
    // Matches the Tauri template grammar: single/double quotes group tokens;
...
// exit status. A custom command is handed off like the existing Tauri launcher;
// long-running editors are not waited for or terminated when TaskHub closes.
