...
@Test func refusesRemoteOriginsAndWrongService() throws {
    ...
    #expect(throws: BackendError.self) {
        try BackendHealth(service: "taskhub", protocol: 1, pid: 1, instanceId: "another").validate(instanceID: "ours")
    }
}
...
@Test func configurationResolvesRustDevelopmentRuntime() throws {
    ...
    let config = try BackendConfiguration.current(arguments: ["TaskHub", "--backend-url", "http://127.0.0.1:4321"], environment: [:])
