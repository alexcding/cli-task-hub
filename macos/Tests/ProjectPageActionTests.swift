...
@MainActor @Test(.timeLimit(.minutes(1))) func projectPageActionsAreCoordinatorOwnedAndKeepErrorsLocal() async throws {
    ...
    actions.browserSucceeds = false; model.openExternally(review)
...
func projectPageActionsCancelPendingNavigationWhenTheirContextChanges(change: String) async throws {
    ...
    await gate.waitForStart()
    switch change {
    case "leave": root.navigate(to: .settings)
    ...
    child.retire()
    model.open(model.rows[0]); model.copyLink(model.rows[0]); model.openExternally(model.rows[0])
    model.connect(ProjectPageService()); await model.refresh()
    #expect(model.retired && actions.opened.count == 1 && actions.copied.isEmpty && actions.browsers.isEmpty)
}
...
@MainActor @Test(.timeLimit(.minutes(1))) func projectPageActionFactoryInjectsOpeningDesktopAndClipboardWithoutDashboard() async throws {
    ...
    let native = NativeProjectFeatureFactory(creation: NativeCreationFlowFactory(chooseFolder: { nil }), desktop: actions, copy: actions.copyLink)
    ...
    let coordinator = ProjectCoordinator(model: model)
    model.copyLink(model.rows[1]); model.openExternally(model.rows[1]); model.open(model.rows[1])
    while !model.opening.isEmpty { await Task.yield() }
    #expect(actions.opened.count == 1 && actions.browsers.count == 1 && actions.copied.count == 1)
    coordinator.retire()
}
