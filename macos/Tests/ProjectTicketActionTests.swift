...
@MainActor @Test(.timeLimit(.minutes(1))) func projectTicketCallbacksRespectOwnershipSectionsAndModalGuards() async throws {
    ...
    tickets.open(ticket); tickets.external(ticket); tickets.copyLink(ticket)
    #expect(actions.opened.isEmpty && actions.browsers.isEmpty && actions.copied.isEmpty)
    ...
    tickets.copyLink(ticket); tickets.external(ticket)
    ...
    #expect(actions.copied == ["https://jira.example.test/browse/REC-1"] && actions.browsers.count == 1)
    ...
    runtime.owns = false; tickets.external(ticket)
    #expect(actions.browsers.count == 1)
...
@MainActor @Test(.timeLimit(.minutes(1))) func webBoardCallbacksAreOwnedAndRejectSuspendedRetiredOrForeignLinks() async throws {
    ...
    #expect(actions.browsers.isEmpty)
