...
// The backend linked into the process (crates/taskhub-backend/src/ffi.rs) behind
// the same APIClient the app uses everywhere.
struct EmbeddedBackendTests {
    private func temporaryDirectory() -> URL {
        FileManager.default.temporaryDirectory.appendingPathComponent("taskhub-embedded-\(UUID().uuidString)", isDirectory: true)
    }
    ...
    @Test func startFailureIsReportedNotFatal() async throws {
        let file = FileManager.default.temporaryDirectory.appendingPathComponent("taskhub-embedded-file-\(UUID().uuidString)")
