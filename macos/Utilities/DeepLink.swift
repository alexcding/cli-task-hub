...
struct TaskHubRouter: DeepLinkRouting {
    ...
    func deepLink(for url: URL) -> DeepLink? {
        guard url.absoluteString.utf8.count <= 2048,
              ...
              components.scheme?.lowercased() == "taskhub", components.host?.lowercased() == "app",
    ...
    func url(for deepLink: DeepLink) -> URL? {
        ...
        return URL(string: "taskhub://app/" + segments.joined(separator: "/"))
    }
...
struct RootRouteHandler: DeepLinkRouteHandling {
    private let routes: [String: SidebarDestination] = ["overview": .overview, "activity": .activity,
