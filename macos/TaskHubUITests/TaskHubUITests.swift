...
final class TaskHubUITests: XCTestCase {
    ...
    func testContextPageFindNavigationCloseAndNewSessionSheet() throws {
        ...
        app.textFields["Title (optional)"].click(); app.typeText("Cancelled session")
        app.buttons["Cancel"].click()
        let dismissed = expectation(for: NSPredicate(format: "exists == false"), evaluatedWith: app.sheets.firstMatch)
        wait(for: [dismissed], timeout: 5)
        app.typeKey("n", modifierFlags: .command)
        XCTAssertTrue(app.textFields["Title (optional)"].waitForExistence(timeout: 5))
        XCTAssertEqual(app.textFields["Title (optional)"].value as? String, "")
        app.buttons["Cancel"].click()
