...
# libsqlite3-sys → rusqlite → taskhub-backend recompiling on nearly every run
...
# dirs ahead of the login PATH, and taskhub-vt/build.rs compiles its shim with a
...
TASKHUB_CARGO_PATH="$HOME/.cargo/bin:/usr/bin:/bin:/usr/sbin:/sbin:/opt/homebrew/bin:/usr/local/bin"

cargo_build() {
  env -i \
    PATH="$TASKHUB_CARGO_PATH" HOME="$HOME" \
