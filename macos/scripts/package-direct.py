#!/usr/bin/env python3
"""Stage a bundled TaskHub app for local review or signed direct distribution.
...
def sign_app(app, identity, local):
    ...
    run(*common, "--entitlements", config / "TaskHub.entitlements", app)
...
def main():
    ...
    if not app.is_dir() or app.suffix != ".app":
        parser.error("--app must be a completed TaskHub.app bundle")
    info = plistlib.loads((app / "Contents/Info.plist").read_bytes())
    if info.get("CFBundleIdentifier") != "com.alexcding.taskhub":
    ...
    with tempfile.TemporaryDirectory(prefix=".taskhub-package-", dir=destination.parent) as temporary:
        ...
        disk = stage / "disk"
        disk.mkdir()
        run("ditto", staged_app, disk / "TaskHub.app")
        (disk / "Applications").symlink_to("/Applications", target_is_directory=True)
        dmg = result / "TaskHub.dmg"
        run("hdiutil", "create", "-volname", "TaskHub", "-srcfolder", disk,
            "-format", "UDZO", "-ov", dmg)
        if not args.local:
    ...
    print(f"Prepared {destination / 'TaskHub.app'}")
