...
@MainActor private final class RecordingAppPlatform: AppPlatformFactory {
    ...
    func viewer(desktop: any DesktopActions, dialogs: BrowserDialogCoordinator,
        ...
        viewerCreations += 1
        return native.viewer(desktop: desktop, dialogs: dialogs, documents: documents, close: close)
    }
    ...
    func pageActions(open: @escaping (OpenPageRequest) async throws -> Void,
        ...
        actionCreations += 1; return native.pageActions(open: open, desktop: desktop, copy: copy)
    }
}
...
@MainActor @Test func appPlatformFactoryOwnsScratchSessionAndRestartConstruction() async throws {
    ...
    let record = WorkspaceSession(id: "injected-session", projectId: "project", workspace: "/tmp",
        worktree: "/tmp/taskhub-injected-worktree", title: "Injected session", branch: "", url: "", createdAt: nil, pinned: false)
