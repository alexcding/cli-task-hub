...
func notificationActivityNavigationUsesRootQueueAndRetiredCallbacksCannotNavigate() async throws {
    ...
    #expect(root.selection == .overview && root.pendingDeepLink?.destination == .activity && windows == 1)
    root.presentNewProject(service: ProjectPageService()) { _ in }
    root.setRoutingReady(true)
    #expect(root.selection == .overview)
    let id = try #require(root.sheet?.id)
    root.dismissSheet(id: id)
    while root.pendingDeepLink != nil { await Task.yield() }
    #expect(root.selection == .activity && runtime.activations == 1)
    let callback = model.onAction
    ...
    #expect(model.retired && child.retired && windows == 1)
    root.notificationCoordinator?.retire()
}
