...
@MainActor @Test func dashboardGroupsReviewOrbitFiltersAndRetainsSnapshotOnFailure() async throws {
    ...
    let actions = ProjectPageActions(); actions.browserSucceeds = false
    ...
    #expect(model.navigation.error != nil && model.error == nil)
...
@MainActor @Test(.timeLimit(.minutes(1))) func dashboardCoordinatorOwnsVisibleRowsAndPreservesFeedbackAcrossSnapshotReads() async throws {
    ...
    root.navigate(to: .settings); model.open(row); model.copyLink(row)
...
@MainActor @Test(.timeLimit(.minutes(1))) func dashboardNewerOpenSupersedesOlderAndReleasedCoordinatorCannotAct() async throws {
    ...
    root = nil
    model.open(first); model.copyLink(first); model.openExternally(first)
    #expect(actions.opened.count == 2 && actions.copied.isEmpty && actions.browsers.isEmpty)
    child.retire()
}
...
@MainActor @Test(.timeLimit(.minutes(1))) func dashboardCachedLinksKeepBrowserAndCopyAvailableWhileDisconnected() async throws {
    ...
    await model.stop()
    model.open(row)
    #expect(actions.opened.isEmpty && model.navigation.error == "Connect to open pull requests in TaskHub.")
    model.copyLink(row); model.openExternally(row)
    #expect(actions.copied == [row.url.absoluteString] && actions.browsers == [row.url] && model.navigation.error == nil)
    root.dashboardCoordinator?.retire()
    model.copyLink(row); model.openExternally(row)
