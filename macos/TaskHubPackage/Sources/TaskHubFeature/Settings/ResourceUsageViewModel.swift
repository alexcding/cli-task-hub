...
@MainActor @Observable final class ResourceUsageViewModel {
    struct Row: Identifiable {
        ...
        var memory: String { ByteCountFormatter.string(fromByteCount: Int64(clamping: process.residentBytes), countStyle: .memory) }
    ...
    var memory: String {
        ...
        let total = rows.reduce(UInt64(0)) { min(maximum, $0 + min(maximum, $1.process.residentBytes)) }
