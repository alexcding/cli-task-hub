...
@MainActor final class BrowserControlsCoordinator {
    ...
    init(desktop: any DesktopActions, canPerform: @escaping () -> Bool = { true }) {
        self.desktop = desktop; self.canPerform = canPerform
    }
    ...
    func bind(_ model: BrowserControlsViewModel, page: any BrowserControlling, isOwned: @escaping () -> Bool) {
        ...
        model.onAction = { [weak self, weak model, weak page] action in
            ...
            case .openExternally:
                guard let url = safeWebURL(page.url) else { return }
                model.externalOpenCompleted(desktop.openBrowser(url))
            }
        }
    }
}
