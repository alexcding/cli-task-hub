...
(function () {
  ...
  //     copy/paste still works); the app pops the real macOS menu via popupMenu below.
  (function () {
    var css =
  ...
  window.taskhub = {
    ...
    // IDE chip right-click (terminal toolbar): the chip's own two actions. Both are renderer
    // state (the active tab's build terminal), so this returns the chosen id only.
    ideMenu: function (ctx) {
      ctx = ctx || {};
      var items = [];
      if (ctx.hasIde) items.push({ id: 'open', label: 'Open in ' + (ctx.ideLabel || 'IDE') });
      if (ctx.hasRun) items.push({ id: 'run', label: ctx.running ? 'Stop Build' : 'Run' });
      return popupMenu(items);
    },
