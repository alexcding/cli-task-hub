...
fn menuitem_set_icon(menuitem: &NSMenuItem, icon: Option<&Icon>) {
    if let Some(icon) = icon {
        unsafe {
            let (_pw, ph) = icon.inner.get_size();
            // PATCHED: no 18px cap. Display at half the pixel size so a 2x image is retina-crisp
            // at its logical size, and the menu row grows to fit (full-size menu-row images).
            let nsimage = icon.inner.to_nsimage(Some(ph as f64 / 2.0));
