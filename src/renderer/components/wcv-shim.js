...
// The in-page context menu (components/menu.js appends one .status-menu to <body>) is DOM too, so
// the native webview paints over it — the toolbar's "+" menu drops down across the page and its
// lower half vanishes. Unlike a modal, a menu covers a small area, so the page is hidden only while
// the menu actually OVERLAPS this shim: a menu popped over the sidebar or the terminal leaves the
// embedded page alone.
const menuRect = () => document.querySelector('.status-menu')?.getBoundingClientRect() || null;
const overlaps = (a, b) => !!b && a.left < b.right && a.right > b.left && a.top < b.bottom && a.bottom > b.top;
// Every live shim, so a modal or menu opening/closing can push its visibility change synchronously
// (the rAF loop is throttled when the renderer isn't painting) rather than wait for the next frame.
// Dialogs append/remove a backdrop under <body> (confirm, new-session) and openMenu appends its
// .status-menu there too; the static #modal toggles its inline display — watch exactly those, not
// the whole tree. Installed lazily by the first shim (the DOM is guaranteed by then;
// module-evaluation order is not). The observer callback is a microtask, so a change lands before
// the next paint without waiting on the (possibly throttled) rAF loop.
const shims = new Set();
...
export function createWcvShim() {
  ...
  // Shown ⇔ displayed and laid out (offsetParent null ⇒ this or an ancestor is display:none), no
  // modal is open, and no in-page menu is drawn over this shim's rect. The native webview is a child
  // of the WINDOW, painted above every DOM layer, so neither a .modal-backdrop (project modal,
  // confirm / new-session dialogs) nor a .status-menu can overlay it — hide the page for that
  // element's lifetime instead; the rAF loop re-shows it the frame the element is gone.
  const isVisible = (r) => el.style.display !== 'none' && el.offsetParent !== null && !modalOpen()
    && !overlaps(r, menuRect());
