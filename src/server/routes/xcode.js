...
async function schemeDefinitionFiles(target) {
  if (!targetArgs(target).length) return [];
  const directories = [path.join(target, 'xcshareddata', 'xcschemes')];
  const userData = path.join(target, 'xcuserdata');
  try {
    const users = await fsp.readdir(userData, { withFileTypes: true });
    for (const user of users) if (user.isDirectory()) directories.push(path.join(userData, user.name, 'xcschemes'));
  } catch { /* A project does not need user-specific schemes. */ }
  const files = [];
  for (const directory of directories) {
    try {
      for (const entry of await fsp.readdir(directory, { withFileTypes: true })) {
        if (entry.isFile() && entry.name.endsWith('.xcscheme')) files.push(path.join(directory, entry.name));
      }
    } catch { /* This scheme directory is optional. */ }
  }
  return files.sort();
}

// `xcodebuild -list` mixes runnable app schemes with dependency and test schemes. Scheme XML is
// cheap to inspect and identifies entries Xcode builds for Run without loading every scheme's
// project graph. If no definitions are available (for example an autogenerated package scheme),
// keep Xcode's list rather than guessing and hiding everything.
async function filterRunnableSchemes(target, schemes) {
  const files = await schemeDefinitionFiles(target);
  if (!files.length) return schemes;
  const runnable = new Set();
  await Promise.all(files.map(async file => {
    const xml = await fsp.readFile(file, 'utf8');
    const entries = xml.match(/<BuildActionEntry\b[\s\S]*?<\/BuildActionEntry>/g) || [];
    if (entries.some(entry => /buildForRunning\s*=\s*"YES"/.test(entry)
      && /BuildableName\s*=\s*"[^"]+\.app"/.test(entry))) {
      runnable.add(path.basename(file, '.xcscheme'));
    }
  }));
  return schemes.filter(scheme => runnable.has(scheme));
}
...
function register(app) {
  ...
  app.get(ROUTES.XCODE_SCHEMES, async (req, res) => {
    ...
    try {
      ...
      const schemes = await filterRunnableSchemes(t.target, info.schemes || []);
      res.json({
        target: t.target, name: info.name || path.basename(t.target),
        schemes, targets: info.targets || [], configurations: info.configurations || [],
      });
    } catch (e) { res.status(500).json({ error: e.message }); }
  });
...
module.exports = { register, parseRuntime, targetArgs, filterRunnableSchemes };
