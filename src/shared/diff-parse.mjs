...
function cleanPath(p, stripPrefix = true) {
  let s = (p || '').trim();
  ...
  return stripPrefix ? s.replace(/^[ab]\//, '') : s;
}
...
export function parseDiff(text) {
  ...
  for (const ln of text.split('\n')) {
    ...
    else if (ln.startsWith('rename from ')) { f.oldPath = cleanPath(ln.slice(12), false); f.status = 'renamed'; }
    else if (ln.startsWith('rename to '))   { f.newPath = cleanPath(ln.slice(10), false); f.status = 'renamed'; }
...
// Git's patch header uses C quoting, not JSON escapes. Encode special filenames
// byte-by-byte so tabs, newlines, quotes and non-ASCII names round-trip through apply.
function quotePatchPath(value) {
  if (!/[\s"\\\x00-\x1f\x7f]/.test(value)) return value;
  return '"' + Array.from(new TextEncoder().encode(value), byte =>
    byte === 34 || byte === 92 ? '\\' + String.fromCharCode(byte)
      : byte < 32 || byte >= 127 ? '\\' + byte.toString(8).padStart(3, '0')
...
export function hunkPatch(f, h) {
  ...
  const out = [`--- ${quotePatchPath(oldSide)}`, `+++ ${quotePatchPath(newSide)}`, h.header];
...
export function blockPatch(f, h, target) {
  ...
  const out = [`--- ${quotePatchPath("a/" + patchPath(f))}`, `+++ ${quotePatchPath("b/" + (f.newPath || f.oldPath))}`];
  const body = [];
  let oldC = 0, newC = 0;
  h.lines.forEach((l, i) => {
