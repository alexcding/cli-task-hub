...
function refresh() {
  inflight ??= (async () => {
    ...
    // All null means ccusage itself failed — keep the previous snapshot if we have one.
    if (claude || codex || block || !cache) cache = { claude, codex, block, limits, asOf: new Date().toISOString() };
    fetchedAt = Date.now();
