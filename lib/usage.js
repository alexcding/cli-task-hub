...
function refresh() {
  inflight ??= (async () => {
    const [claude, codex, block, limits] = await Promise.all([
    ...
    // the OAuth endpoint). A transient failure must NOT blank the Session/Weekly bars —
    // keep the last good value, mirroring the poller's stale-while-revalidate snapshots.
    // Per-agent stats (claude/codex) similarly hold their last good value on a null.
    if (claude || codex || block || limits || !cache) {
      cache = {
        claude: claude ?? cache?.claude ?? null,
        codex:  codex  ?? cache?.codex  ?? null,
        block:  block  ?? cache?.block  ?? null,
        limits: limits ?? cache?.limits ?? null,
