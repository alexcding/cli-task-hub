...
function getTabs() {
  ...
  return {
    tabs: rows.map(r => ({ kind: r.kind, title: r.title, url: r.url, repo: r.repo || '', branch: r.branch || '', prSplit: !!r.pr_split })),
    active: active ? active.url : null,
  };
}
const _insertTab = db.prepare(
  `INSERT INTO tabs (url, kind, title, repo, branch, pr_split, position, active) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`
);
  ...
  try {
    ...
    tabs.forEach((t, i) => {
      ...
      _insertTab.run(t.url, t.kind === 'jira' ? 'jira' : 'github', t.title || t.url,
        t.repo || '', t.branch || '', t.prSplit ? 1 : 0, i, active && t.url === active ? 1 : 0);
    });
    db.exec('COMMIT');
