...
const jiraSnapshotPath = path.join(dataDir, 'taskhub-jira-snapshot.json'); // cached Jira tickets (volatile, rebuilt by sync)

const uuid = () => crypto.randomUUID();

// ── Project shape ───────────────────────────────────────────────────────────────
// { id, name, color, repo, workspace, jql, mergeTransition, created_at }
...
const PROJECT_FIELDS = ['name', 'color', 'repo', 'workspace', 'jql', 'mergeTransition'];
...
// ── Jira snapshot store (cached ticket data the UI reads) ───────────────────────
// Shape: { [id]: { items: [...lean tickets], jql, lastSynced: ISO, error: string|null } }
// id is a project UUID for per-project queries, or MY_TICKETS_ID ('@me') for the
// global "assigned to me" feed. Same SWR model as the PR snapshot above.
let _jsnap = null;
function loadJSnap() {
  if (_jsnap) return _jsnap;
  try { _jsnap = JSON.parse(fs.readFileSync(jiraSnapshotPath, 'utf8')); } catch { _jsnap = {}; }
  return _jsnap;
}
function saveJSnap() {
  fs.mkdirSync(path.dirname(jiraSnapshotPath), { recursive: true });
  fs.writeFileSync(jiraSnapshotPath, JSON.stringify(_jsnap));
}
const getJiraSnapshot     = id => loadJSnap()[id] || null;
const getAllJiraSnapshots = () => ({ ...loadJSnap() });
const setJiraSnapshot     = (id, snap) => { loadJSnap()[id] = snap; saveJSnap(); };
const deleteJiraSnapshot  = id => { const s = loadJSnap(); if (id in s) { delete s[id]; saveJSnap(); } };
