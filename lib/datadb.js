...
// ── One-time migration from the old JSON sidecar caches ─────────────────────────
// On the first launch after upgrading, seed the tables from taskhub-snapshot.json /
// taskhub-jira-snapshot.json so the UI shows last-known data immediately instead of a
// brief "Loading…" while the first sync runs. Best-effort: missing/garbled files are
// ignored, and we only import into empty tables so we never clobber fresher cache.
function migrateFromJson() {
  try {
    if (db.prepare('SELECT COUNT(*) n FROM pr_snapshots').get().n === 0) {
      const old = parse(fs.readFileSync(path.join(dataDir, 'taskhub-snapshot.json'), 'utf8'), {});
      for (const [id, snap] of Object.entries(old)) if (snap) setSnapshot(id, snap);
    }
  } catch { /* no old PR cache */ }
  try {
    if (db.prepare('SELECT COUNT(*) n FROM jira_snapshots').get().n === 0) {
      const old = parse(fs.readFileSync(path.join(dataDir, 'taskhub-jira-snapshot.json'), 'utf8'), {});
      for (const [id, snap] of Object.entries(old)) if (snap) setJiraSnapshot(id, snap);
    }
  } catch { /* no old Jira cache */ }
}
migrateFromJson();
