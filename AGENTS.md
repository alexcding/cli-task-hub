...
- `crates/taskhub-backend/src/poller.rs` is the **only** thing that calls `gh`. Every
...
**Open `macos/TaskHub.xcodeproj` and press ⌘R. That is the whole workflow.**
...
# The Swift suites (275 tests across TaskHubTests + TaskHubUITests)
...
- **Swift Testing does not match `-only-testing:TaskHubTests/someFunctionName`.** It runs
...
- `TASKHUB_DATA_DIR` overrides the data directory (default
  `~/Library/Application Support/TaskHub`).

## Files

**Backend** (`crates/taskhub-backend/src/`):
...
**Terminal** (`crates/taskhub-ptyd`, `crates/taskhub-vt`): a detached PTY daemon whose
...
- **Schema is `CREATE TABLE IF NOT EXISTS`** in the three `schema_*.sql` files — no
  migration framework. `data.db` and `logs.db` are regenerable caches; **`taskhub.db` is
