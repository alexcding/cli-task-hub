...
fn main() {
    println!("cargo:rerun-if-env-changed=TASKHUB_GHOSTTY_VT_DIR");
    println!("cargo:rerun-if-changed=native/terminal.c");
    let root = PathBuf::from(env::var("CARGO_MANIFEST_DIR").unwrap());
    let runtime = env::var_os("TASKHUB_GHOSTTY_VT_DIR")
        .map(PathBuf::from)
    ...
    fs::copy(archive, out.join("libtaskhub-ghostty-core.a"))
    ...
    run(Command::new("ar")
        .arg("crs")
        .arg(out.join("libtaskhub-vt-shim.a"))
        .arg(object));
    println!("cargo:rustc-link-search=native={}", out.display());
    println!("cargo:rustc-link-lib=static=taskhub-vt-shim");
    println!("cargo:rustc-link-lib=static=taskhub-ghostty-core");
