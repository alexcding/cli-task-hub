...
use taskhub_backend::{build_app, recovery, shutdown_signal, AppState, Database};
...
async fn main() -> Result<()> {
    ...
    let port = env::var("PORT")
        ...
        .unwrap_or(3000);
    let data_dir = env::var_os("TASKHUB_DATA_DIR")
        .map(PathBuf::from)
    ...
    let _native_lease = if env::var("TASKHUB_PACKAGED").as_deref() == Ok("1") {
    ...
    let state = AppState::new(database, env::var("TASKHUB_INSTANCE_ID").ok());
    ...
    tracing::info!("TaskHub Rust backend running at http://127.0.0.1:{bound_port}");
    axum::serve(listener, app)
        .with_graceful_shutdown(shutdown_signal())
        .await
        .context("serve TaskHub backend")?;
...
fn default_data_dir() -> PathBuf {
    env::var_os("HOME")
        ...
        .join("Library/Application Support/TaskHub")
}
