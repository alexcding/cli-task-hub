...
pub async fn put_file(headers: HeaderMap, Json(body): Json<Value>) -> ApiResult<Value> {
    ...
    let temporary = canonical.with_file_name(format!(".taskhub-save-{}", Uuid::new_v4()));
...
fn resolve_launch(root: &Path, rel: &str, kind: &str) -> Result<(PathBuf, &'static str), ApiError> {
...
fn xcode_request(query: &LocalQuery) -> Result<(PathBuf, PathBuf), ApiError> {
    let raw = query
        .path
        .as_deref()
        .filter(|v| !v.is_empty())
...
pub async fn xcode_simulators(headers: HeaderMap) -> ApiResult<Value> {
    if foreign_origin(&headers) {
        return Err(ApiError::forbidden("forbidden"));
    }
    let raw = cli::run(
    ...
    .await
    .map_err(ApiError::internal)?;
    let value: Value = serde_json::from_str(&raw).map_err(ApiError::internal)?;
    let mut list = Vec::new();
    if let Some(devices) = value["devices"].as_object() {
        for (runtime, items) in devices {
            let label = runtime
                .split("SimRuntime.")
                .nth(1)
                .unwrap_or(runtime)
                .replace('-', ".");
            for device in items.as_array().into_iter().flatten() {
                if device["isAvailable"].as_bool() != Some(false) {
                    list.push(json!({"udid":device["udid"],"name":device["name"],"platform":label.split('.').next().unwrap_or(""),"runtime":label,"state":device["state"],"lastUsedAt":device["lastUsedAt"]}))
                }
            }
        }
    }
...
pub async fn xcode_build_settings(
    headers: HeaderMap,
    Query(query): Query<LocalQuery>,
) -> ApiResult<Value> {
    if foreign_origin(&headers) {
    ...
    let sim = query
        ...
        .ok_or_else(|| ApiError::bad_request("sim (a simulator UDID) required"))?;
    let (root, target) = xcode_request(&query)?;
