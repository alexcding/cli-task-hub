...
/// Receives one response per `taskhub_backend_request`, on a runtime thread.
...
fn start(data_dir: PathBuf, packaged: bool, instance_id: Option<String>) -> anyhow::Result<TaskHubBackend> {
    use anyhow::Context;
    ...
    tracing::info!("TaskHub backend embedded; loopback listener at http://127.0.0.1:{port}");
    Ok(TaskHubBackend {
        runtime,
        state,
        router,
...
/// `taskhub_string_free`). `instance_id` may be null.
///
/// # Safety
/// `data_dir` and `instance_id` are NUL-terminated strings; `out` and `error`
/// are writable or null.
#[no_mangle]
pub unsafe extern "C" fn taskhub_backend_start(
    data_dir: *const c_char,
    packaged: i32,
    instance_id: *const c_char,
    out: *mut *mut TaskHubBackend,
    error: *mut *mut c_char,
) -> i32 {
    let result = catch_unwind(AssertUnwindSafe(|| -> Result<TaskHubBackend, String> {
...
///
/// # Safety
/// `backend` came from `taskhub_backend_start` and has not been stopped.
#[no_mangle]
pub unsafe extern "C" fn taskhub_backend_port(backend: *const TaskHubBackend) -> u16 {
...
#[no_mangle]
pub unsafe extern "C" fn taskhub_backend_request(
    backend: *const TaskHubBackend,
    method: *const c_char,
...
/// `taskhub_backend_unsubscribe`, at `taskhub_backend_stop`, or if the event
/// channel closes.
///
/// # Safety
/// `ctx` stays valid until `dropped` has run.
#[no_mangle]
pub unsafe extern "C" fn taskhub_backend_subscribe(
    backend: *const TaskHubBackend,
    ctx: *mut c_void,
...
/// `backend` came from `taskhub_backend_start` and has not been stopped.
#[no_mangle]
pub unsafe extern "C" fn taskhub_backend_unsubscribe(backend: *const TaskHubBackend, id: u64) {
    if backend.is_null() {
        return;
    }
...
///
/// # Safety
/// `backend` came from `taskhub_backend_start` and is stopped exactly once.
#[no_mangle]
pub unsafe extern "C" fn taskhub_backend_stop(backend: *mut TaskHubBackend) {
    if backend.is_null() {
        return;
    }
    ...
    let TaskHubBackend { runtime, .. } = *backend;
...
#[no_mangle]
pub unsafe extern "C" fn taskhub_string_free(value: *mut c_char) {
    if !value.is_null() {
...
mod tests {
    ...
    fn call(backend: *const TaskHubBackend, method: &str, target: &str, body: &str) -> Reply {
        let (sender, receiver) = mpsc::channel();
        let sender = Box::new(sender);
        let method = CString::new(method).unwrap();
        let target = CString::new(target).unwrap();
        unsafe {
            taskhub_backend_request(
                backend,
                method.as_ptr(),
                target.as_ptr(),
                body.as_ptr(),
    ...
    fn start_temp() -> (*mut TaskHubBackend, tempfile::TempDir) {
        let dir = tempfile::tempdir().unwrap();
        let path = CString::new(dir.path().to_str().unwrap()).unwrap();
        let instance = CString::new("test-instance").unwrap();
        let mut handle: *mut TaskHubBackend = std::ptr::null_mut();
        let mut error: *mut c_char = std::ptr::null_mut();
        let code = unsafe { taskhub_backend_start(path.as_ptr(), 0, instance.as_ptr(), &mut handle, &mut error) };
    ...
    fn dispatches_requests_in_process_and_serves_a_loopback_port() {
        ...
        let port = unsafe { taskhub_backend_port(backend) };
        assert!(port > 0);
        ...
        assert!(over_tcp.unwrap().contains("\"service\":\"taskhub\""));
        ...
        unsafe {
            taskhub_backend_request(backend, method.as_ptr(), target.as_ptr(), std::ptr::null(), 0,
                                    &*sender as *const _ as *mut c_void, Some(on_response));
            taskhub_backend_stop(backend);
        }
    ...
    fn subscriptions_deliver_broadcasts_and_release_their_context() {
        ...
        let id = unsafe { taskhub_backend_subscribe(backend, ctx, Some(on_event), Some(on_dropped)) };
        assert!(id > 0);
        ...
        unsafe { taskhub_backend_unsubscribe(backend, id) };
        ...
        let ctx = Box::into_raw(Box::new(sender)) as *mut c_void;
        let drops = DROPS.load(Ordering::SeqCst);
        assert!(unsafe { taskhub_backend_subscribe(backend, ctx, Some(on_event), Some(on_dropped)) } > 0);
        unsafe { taskhub_backend_stop(backend) };
        assert_released(&receiver, drops + 1);
    }
    ...
    fn the_swift_header_declares_every_exported_function() {
        ...
        let header = include_str!("../../../macos/Vendor/TaskHubBackend/taskhub_backend.h");
        ...
        for name in exported {
            assert!(header.contains(&format!(" {name}(")), "{name} is missing from taskhub_backend.h");
        }
    }
    ...
    fn start_reports_errors_instead_of_panicking() {
        ...
        let mut handle: *mut TaskHubBackend = std::ptr::null_mut();
        let mut error: *mut c_char = std::ptr::null_mut();
        let code = unsafe { taskhub_backend_start(path.as_ptr(), 0, std::ptr::null(), &mut handle, &mut error) };
        assert_eq!(code, 1);
        assert!(handle.is_null());
        assert!(!error.is_null());
        assert!(unsafe { CStr::from_ptr(error) }.to_string_lossy().contains("create data directory"));
        unsafe { taskhub_string_free(error) };
    }
}
