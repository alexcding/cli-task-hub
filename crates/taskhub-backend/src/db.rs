...
impl Database {
    pub fn open(data_dir: &Path) -> Result<Self> {
        ...
        let durable = open_db(&data_dir.join("taskhub.db"))?;
...
fn migrate_legacy_name(data_dir: &Path) {
    let target = data_dir.join("taskhub.db");
    if target.exists() || !data_dir.join("config.db").exists() {
        return;
    }
    for suffix in ["", "-wal", "-shm", "-journal"] {
        let from = data_dir.join(format!("config.db{suffix}"));
        if from.exists() {
            let _ = fs::rename(&from, data_dir.join(format!("taskhub.db{suffix}")));
        }
    }
}
