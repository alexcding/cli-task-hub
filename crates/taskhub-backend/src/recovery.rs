...
fn kind(name: &str) -> Option<&'static str> {
    match name {
        "taskhub.db" | "config.db" | "logs.db" => Some("sqlite"),
...
pub fn backup(source: &Path, destination: &Path) -> Result<Manifest> {
    ...
    let primary = if regular_file(&source, "taskhub.db")?.is_some() {
        "taskhub.db"
    } else if regular_file(&source, "config.db")?.is_some() {
        "config.db"
    } else {
        bail!("No durable taskhub.db or legacy config.db was found")
    };
...
pub fn verify(source: &Path) -> Result<Manifest> {
    ...
    for entry in &manifest.files {
        ...
        seen.contains("taskhub.db") != seen.contains("config.db"),
...
pub fn prepare_packaged(data: &Path) -> Result<NativeLease> {
    ...
    let has_data =
        regular_file(&data, "taskhub.db")?.is_some() || regular_file(&data, "config.db")?.is_some();
...
fn release_identity() -> Result<String> {
        ...
        .filter(|p| p.file_name().is_some_and(|n| n == "Helpers"))
    {
        ...
        for name in ["MacOS/TaskHub", "Info.plist"] {
...
pub fn run_command(arguments: &[std::ffi::OsString]) -> Result<bool> {
    ...
    let usage = "Usage: taskhub-backend [backup DATA_DIR NEW_SNAPSHOT_DIR | verify SNAPSHOT_DIR | restore SNAPSHOT_DIR NEW_DATA_DIR]";
